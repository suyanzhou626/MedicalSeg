...
def eval(opt):
    ...
    method = opt.pred_path.split('/')[-5] # for model name
    ...
    for dataset in tqdm.tqdm(opt.datasets, desc='Expr - ' + method, total=len(opt.datasets), position=0, bar_format='{desc:<30}{percentage:3.0f}%|{bar:50}{r_bar}'):
        ...
        gt_path = os.path.join(opt.gt_path, dataset, 'masks')
        ...
        csv = os.path.join(opt.result_path, 'full_result_' + dataset + '.csv')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pred_path', type=str, required=True, help='pred path')
    parser.add_argument('--gt_path', type=str, required=True, help='gt path')
    parser.add_argument('--datasets', type=lambda s: [item for item in s.split(',')],
                        required=True, help='datasets to eval model performance.')
